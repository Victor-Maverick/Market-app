...
const InputField = ({
    ...
    return (
        <div className="relative w-full flex flex-col">
            <label
                htmlFor={id}
                className={`absolute left-4 transition-all ${isFocused || value
            ...
            <input
                ...
                className={`px-4 h-[58px] w-full border-[1.5px] border-[#D1D1D1] rounded-[14px] outline-none focus:border-[2px] focus:border-[#022B23] ${isFocused || value
...
const PayoutRequestModal = ({
    ...
    const handleSaveDetails = async () => {
        ...
        try {
            ...
            const errorMessage = err.response?.data || err.message || "An error occurred while updating account details";
    ...
    const handleContinue = async () => {
        ...
        try {
            ...
            if (response.data) {
                ...
                const errorMsg = "Failed to request payout";
                setError(errorMsg);
                toast.error(errorMsg);
            }
        } catch (err: any) {
            const errorMessage = err.response?.data || err.message || "An error occurred while requesting payout";
            setError(errorMessage);
            toast.error(errorMessage);
        } finally {
            setIsSubmitting(false);
